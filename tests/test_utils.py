...
def test_get_years_in_path(tmp_path):
    ...
    with pytest.raises(ValueError):
        themeda_preproc.utils.get_years_in_path(path=tmp_path)

    # unless silenced
    themeda_preproc.utils.get_years_in_path(path=tmp_path, error_if_no_years=False)
    ...
    inferred_years = themeda_preproc.utils.get_years_in_path(path=tmp_path)
    ...
    # unless silenced
    inferred_years = themeda_preproc.utils.get_years_in_path(
...
def test_num_str_to_int():
    # OK
    assert themeda_preproc.utils.num_str_to_int("5") == 5
    assert themeda_preproc.utils.num_str_to_int("5.") == 5
    assert themeda_preproc.utils.num_str_to_int("-5") == -5

    with pytest.raises(ValueError):
        themeda_preproc.utils.num_str_to_int("five")
    with pytest.raises(ValueError):
        themeda_preproc.utils.num_str_to_int("5.5")
...
def test_transform_shape():
    ...
    converted = themeda_preproc.utils.transform_shape(
        src_crs=4326,
        dst_crs=3577,
        shape=example_4326,
    )
    ...
    backwards = themeda_preproc.utils.transform_shape(
        src_crs=3577,
        dst_crs=4326,
        shape=converted,
    )
